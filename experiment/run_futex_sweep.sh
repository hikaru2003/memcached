...
#   memcached_debug_futex バイナリ (debug/futex-count ブランチ) の SIGUSR2 ハンドラを使用。
#
...
#   計測フロー (各 PAUSE 値ごと):
#     1. memcached 起動
#     2. warmup → SIGUSR2 でカウンタリセット
#     3. mutilate DURATION 秒 → SIGUSR2 でカウンタ読み出し
#     4. QPS と futex_count から futex_per_op を算出
#     5. memcached を kill
#
# Parameters (env vars):
#   MEMCACHED_BIN   - debug バイナリ                            (default: ./memcached_debug_futex)
...
#
# Output:
#   experiment/results/futex_sweep_mc{MC}_mut{MUT}/
#     run_info.md       - 実験日時・パラメータ
#     summary.md        - PAUSE値別 futex_per_op / QPS テーブル
#     raw.csv           - per-run 生データ (pause_count,run,QPS,futex_count,futex_per_op,...)
#     raw/run_P{pause}_{run}.log    - mutilate ログ
#     raw/mc_P{pause}.log          - memcached stderr (SPINLOCK_FUTEX_COUNT を含む)
#
# Prerequisites:
...
_BASE="experiment/results/futex_sweep_mc${MC_THREADS}_mut${MUT_THREADS}"
RESULT_DIR="$_BASE"
_i=2; while [ -d "$RESULT_DIR" ]; do RESULT_DIR="${_BASE}_run${_i}"; _i=$((_i+1)); done
mkdir -p "$RESULT_DIR/raw"

n_pause=$(echo "$PAUSE_VALUES" | wc -w)
# 各 PAUSE 値: warmup + RUNS×DURATION (warmup は memcached 再起動ごとに 1 回)
...
start_memcached() {
    ...
    for i in $(seq 1 10); do
        ...
        if ss -tnlp 2>/dev/null | grep -q ":$PORT"; then
            echo "[mc] PID=$MC_PID  PAUSE_COUNT=$pause  log=$log"
            return 0
        fi
    done
...
reset_futex_counter() {
    ...
    sleep 0.2  # ハンドラが stderr に書き終わるのを待つ
}

read_futex_count() {
    # SIGUSR2 で現在値を出力させ、最新の "SPINLOCK_FUTEX_COUNT: N" を取得
    kill -USR2 "$MC_PID" 2>/dev/null || true
    sleep 0.2
...
echo "============================================================"
echo " futex sweep  (memcached_debug_futex, SIGUSR2 counter)"
echo "============================================================"
echo " binary      : $MEMCACHED_BIN"
echo " mc_threads  : $MC_THREADS  cpus: $MC_CPUS"
echo " mut         : -T $MUT_THREADS -c $MUT_CONNS -d $DEPTH  (total_conns=$TOTAL_CONNS)"
...
echo " conditions  : ${n_pause} pause vals × ${RUNS} runs  (est. ~${est_min} min)"
...
# summary.md ヘッダ
{
    echo "# futex sweep / $(basename "$MEMCACHED_BIN") / mc=${MC_THREADS} / mutilate -T ${MUT_THREADS} -c ${MUT_CONNS} -d ${DEPTH} -r ${RECORDS} -u ${UPDATE_RATIO} / n=${RUNS}"
    echo ""
    echo "futex_per_op = futex_count / (QPS × DURATION)"
    echo "小さいほどスピンでロック取得 (futex fallback が少ない)"
    echo ""
    echo "| pause_count | mean_QPS | mean_futex_per_sec | mean_futex_per_op | cv_qps% | n |"
    echo "|---|---|---|---|---|---|"
...
for pause_count in $PAUSE_VALUES; do
    ...
    echo "=============================="
    mc_log="$RESULT_DIR/raw/mc_P${pause_count}.log"
    start_memcached "$pause_count" "$mc_log"
    ...
    qps_list="" futex_per_sec_list="" futex_per_op_list=""
    r_avg_list="" r_p99_list="" w_avg_list="" w_p99_list=""
    ...
    for run_idx in $(seq 1 "$RUNS"); do
        ...
        # SIGUSR2 でこのランの futex_count を読み出す（同時にリセット → 次ランへ）
        ...
        echo "$pause_count,$run_idx,$qps,$futex_count,$futex_per_sec,$futex_per_op,$r_avg,$r_p99,$w_avg,$w_p99" \
            >> "$RESULT_DIR/raw.csv"
    done
    ...
    echo "| $pause_count | $mean_qps | $mean_fps | $mean_fpo | $cv_qps | $RUNS |" \
        >> "$RESULT_DIR/summary.md"
