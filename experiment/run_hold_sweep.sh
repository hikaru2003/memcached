...
#   bash experiment/run_hold_sweep.sh
#
# Description:
#   PAUSE count N を変えながらクリティカルセクション実行時間（hold time）を計測する。
#   spinlock_lock で lock 獲得直後、spinlock_unlock で pthread_mutex_unlock 直前に
#   rdtsc を取り、差分をリングバッファに記録する。全ラン後に SIGUSR2 で
#   hold_samples_thread*.bin をダンプして RESULT_DIR/N<n>/ に収集する。
#
#   測定内容: lock 獲得〜unlock 呼び出しまでのサイクル数 (= 純粋な CS 実行時間)
#     M権再取得コスト・trylock リトライ時間は含まない。
#
# Parameters (env vars):
...
#   RUNS                   - 各Nのラン数（hold samplesを蓄積）    (default: 10)
...
#
# Output:
#   experiment/results/hold_YYYYMMDD_HHMMSS/
#     run_info.md                        - 実験パラメータ
#     run.log                            - スクリプト完全ログ
#     N<n>/hold_samples_thread<t>.bin    - 各条件・スレッドの CS 長サンプル（uint64_t, rdtscサイクル）
...
if [ ! -x "$MEMCACHED_BIN" ]; then
    ...
    echo "  Build: git checkout debug/hold-time-v2 && make && cp memcached memcached_hold_debug"
    exit 1
fi
...
RESULT_DIR="experiment/results/hold_${RUN_DATE}"
mkdir -p "$RESULT_DIR"
...
{
    echo "# Run info (hold time / critical section length sweep)"
...
run_one_config() {
    ...
    # RUNS回連続計測。hold samplesはリングバッファに蓄積し続ける
    local total_qps=0
...
echo "============================================================"
echo " hold time (critical section length) sweep"
echo "============================================================"
echo " bin         : $MEMCACHED_BIN"
echo " mc_threads  : $MC_THREADS  cpus: $MC_CPUS"
echo " mut         : -T $MUT_THREADS -c $MUT_CONNS -d $DEPTH -r $RECORDS -u $UPDATE_RATIO"
