...
#   bash experiment/run_unlock_sweep.sh
#
# Description:
#   PAUSE count N を変えながら pthread_mutex_unlock レイテンシを計測する。
#   spinlock_unlock 内で unlock 前後の rdtsc 差分をリングバッファに記録し、
#   全ラン後に SIGUSR2 で unlock_samples_thread*.bin をダンプして
#   RESULT_DIR/N<n>/ に収集する。
#
#   測定内容: pthread_mutex_unlock() の実行時間 (cy)
#     = M権再取得待ち + atomic store + (futex_wake_if_needed)
#   高競合時に伸びる = 他コアの trylock による cacheline 奪取頻度の指標。
#
# Parameters (env vars):
#   MEMCACHED_BIN          - unlock-latency計測バイナリ         (default: ./memcached_unlock_debug)
...
#   RUNS                   - 各Nのラン数（unlock samplesを蓄積）  (default: 10)
#   SPIN_ROUNDS            - trylock試行回数（固定）              (default: 30)
#   PAUSE_PER_ROUND_VALUES - N sweep値
#     default: 40点 [0-18全整数, 20-30 2刻み, 33-60 密, 70-200 疎]
#   PORT                   - memcachedポート                    (default: 11222)
#   MC_CPUS                - memcached CPU affinity             (default: 0-3)
#   WL_CPUS                - mutilate CPU affinity              (default: 4-7)
#
# Output:
#   experiment/results/unlock_YYYYMMDD_HHMMSS/
#     run_info.md                          - 実験パラメータ
#     run.log                              - スクリプトの標準出力/標準エラーの完全ログ
#     N<n>/unlock_samples_thread<t>.bin    - 各条件・スレッドの unlock latency サンプル（uint64_t, rdtscサイクル）
#     N<n>/stats.txt                       - mean QPS
#
# Prerequisites:
#   - ./memcached_unlock_debug: debug/unlock-latency ブランチのビルド
#   - ../mutilate/mutilate_p999 (or set MUTILATE_BIN)
#   - setup_perf_env.sh実施済み（SMT off / performance governor / turbo off）

set -uo pipefail

MEMCACHED_BIN="${MEMCACHED_BIN:-./memcached_unlock_debug}"
...
if [ ! -x "$MEMCACHED_BIN" ]; then
    ...
    echo "  Build: git checkout debug/unlock-latency && make && cp memcached memcached_unlock_debug"
    exit 1
fi
...
RESULT_DIR="experiment/results/unlock_${RUN_DATE}"
mkdir -p "$RESULT_DIR"
LOG_FILE="$RESULT_DIR/run.log"
# 以降の stdout/stderr を tee 経由でログファイルにも書き出す
...
{
    echo "# Run info (unlock latency sweep)"
...
run_one_config() {
    ...
    # RUNS回連続計測。unlock samplesはリングバッファに蓄積し続ける
    local total_qps=0
    ...
    # SIGUSR2 で unlock サンプルをダンプ
    kill -USR2 "$MC_PID"
    sleep 1
    if mv unlock_samples_thread*.bin "$out_dir/" 2>/dev/null; then
        local nfiles
        nfiles=$(ls "$out_dir"/unlock_samples_thread*.bin 2>/dev/null | wc -l)
        echo "  samples dumped: $nfiles files -> $out_dir/"
    else
        echo "[WARN] no unlock_samples_thread*.bin found for $label"
    fi
...
echo ""
echo "次のステップ:"
echo "  統計抽出: python3 experiment/extract_unlock_stats.py --dir $RESULT_DIR"
