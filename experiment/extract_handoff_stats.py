...
#   python3 experiment/extract_handoff_stats.py --dir experiment/results/handoff_YYYYMMDD_HHMMSS
#
# Description:
#   handoff sweep 実験結果ディレクトリ内の .bin ファイルからパーセンタイル統計を抽出し、
#   handoff_summary.csv として保存する。グラフ生成は行わない（plot_handoff_comparison.py で実施）。
#
# Parameters:
#   --dir DIR      handoff sweep 実験結果ディレクトリ
#   --tsc-mhz MHZ TSC周波数 MHz（省略で /proc/cpuinfo から自動取得）
#   --out PATH     出力 CSV パス（省略で <DIR>/handoff_summary.csv）
#
# Output:
#   <DIR>/handoff_summary.csv — 条件ごとのパーセンタイル統計（µs）
...
def load_condition(dirpath):
    """dirpath 内の handoff_samples_thread*.bin を全スレッド合算して返す（cycles）。"""
    files = sorted(glob.glob(os.path.join(dirpath, "handoff_samples_thread*.bin")))
    if not files:
        return None
...
def discover_conditions(base_dir):
    ...
    for name in entries:
        ...
        if os.path.isdir(path) and glob.glob(os.path.join(path, "handoff_samples_thread*.bin")):
...
def main():
    ...
    ap.add_argument("--dir",     required=True, help="handoff sweep 実験結果ディレクトリ")
    ...
    out_path = args.out or os.path.join(args.dir, "handoff_summary.csv")

    conditions = discover_conditions(args.dir)
    if not conditions:
        print(f"[ERROR] No condition directories with handoff_samples_thread*.bin found in {args.dir}",
              file=sys.stderr)
        sys.exit(1)
    ...
    for name, path in conditions:
        ...
        row = {"condition": name, "tsc_mhz": f"{tsc_mhz:.1f}",
