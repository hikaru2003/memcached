...
#
# Description:
#   mutilate の -d (pipeline depth) を変化させ、飽和 QPS への影響を計測する。
#   depth=1 はクローズドループ（デフォルト）。depth>1 でレスポンス待ちなしに
#   複数リクエストを送り続けるため、I/O 律速が軽減される可能性がある。
#
#   外ループ: DEPTH_VALUES をスイープ
#   内ループ: MUT_CONNS_VALUES をスイープ（各 depth での飽和点を探す）
#
# Parameters (env vars):
#   MEMCACHED_BIN     - memcached binary                   (default: ./memcached)
#   MUTILATE_BIN      - mutilate binary                    (default: ../mutilate/mutilate)
#   MC_THREADS        - memcached worker threads           (default: 8)
#   MUT_THREADS       - mutilate client threads            (default: 4)
#   MUT_CONNS_VALUES  - conns/thread sweep 値              (default: "1 4 16 64")
#   DEPTH_VALUES      - pipeline depth sweep 値            (default: "1 2 4 8 16 32")
#   UPDATE_RATIO      - GET:SET 比率                       (default: 0.5)
#   RECORDS           - key range                          (default: 1)
#   WARMUP_SEC        - warmup 秒数                        (default: 10)
#   DURATION          - 計測秒数                           (default: 30)
#   RUNS              - 各設定ごとのラン数                  (default: 3)
...
#
# Output:
#   experiment/results/depth_sweep_t{MC}_T{MT}_r{REC}/
#     run_info.md    - 実験日時・パラメータ
#     summary.md     - depth × conns QPS テーブル + 各 depth 飽和 QPS まとめ
#     raw.csv
#     d{depth}_C{conns}_{run}.log
#
# Prerequisites:
...
MUT_CONNS_VALUES="${MUT_CONNS_VALUES:-1 4 16 64}"
DEPTH_VALUES="${DEPTH_VALUES:-1 2 4 8 16 32}"
...
_BASE="experiment/results/depth_sweep_t${MC_THREADS}_T${MUT_THREADS}_r${RECORDS}"
RESULT_DIR="$_BASE"
...
run_mutilate() {
    local logfile=$1 mut_conns=$2 depth=$3
...
echo "============================================================"
echo " memcached depth sweep (pipeline -d)"
echo "============================================================"
...
echo " depth sweep: $DEPTH_VALUES"
echo " conns sweep: $MUT_CONNS_VALUES  (total = MUT_THREADS × MUT_CONNS)"
echo " results    : $RESULT_DIR"
echo "============================================================"

{
    echo "# depth sweep / t=${MC_THREADS} / mutilate -T ${MUT_THREADS} -r ${RECORDS} -u ${UPDATE_RATIO} / n=${RUNS}"
    echo "# PAUSE_COUNT=${PAUSE_COUNT}"
    echo ""
    echo "| depth | total_conns | mean_QPS | median_QPS | stddev_QPS | cv_pct | r_avg_us | r_p99_us | w_avg_us | w_p99_us | n |"
    echo "|---|---|---|---|---|---|---|---|---|---|---|"
} > "$RESULT_DIR/summary.md"

echo "depth,total_conns,mut_conns,run,QPS,r_avg_us,r_p99_us,w_avg_us,w_p99_us" > "$RESULT_DIR/raw.csv"

# depth ごとに memcached を再起動してコネクション数をスイープ
for depth in $DEPTH_VALUES; do
    ...
    for mut_conns in $MUT_CONNS_VALUES; do
        ...
        echo ""
        echo "  >>> depth=$depth  total_conns=$total_conns"
        ...
        for run_idx in $(seq 1 "$RUNS"); do
            logfile="$RESULT_DIR/d${depth}_C${total_conns}_${run_idx}.log"
            run_mutilate "$logfile" "$mut_conns" "$depth" > /dev/null
            ...
            echo "$depth,$total_conns,$mut_conns,$run_idx,$qps,$r_avg,$r_p99,$w_avg,$w_p99" >> "$RESULT_DIR/raw.csv"
        done
        ...
        echo "| $depth | $total_conns | $mean_qps | $median_qps | $sd_qps | $cv_qps | $mean_r_avg | $mean_r_p99 | $mean_w_avg | $mean_w_p99 | $RUNS |" \
            >> "$RESULT_DIR/summary.md"
    done
...
# 飽和 QPS まとめ（各 depth の最大 total_conns = 飽和点とみなす）
{
    echo ""
    echo "## Saturated QPS summary (at max connections)"
    echo ""
    echo "| depth | sat_QPS | sat_r_avg_us | sat_r_p99_us |"
    echo "|---|---|---|---|"
    max_conns=$(echo "$MUT_CONNS_VALUES" | tr ' ' '\n' | tail -1)
    max_total=$(( MUT_THREADS * max_conns ))
    grep "^| " "$RESULT_DIR/summary.md" | grep "| $max_total |" | \
